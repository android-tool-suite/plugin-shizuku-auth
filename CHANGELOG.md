...
- 仅调整声明式 UI，不改变 Provider 权限、签名或系统调用范围。
...
- 使用插件 SDK 2.0.0 的 Provider 注册契约，不依赖已移除的 API1 Tool 接口；最低宿主 versionCode 为 24。Shizuku 通用进程接口已被上游弃用，当前明确支持 API 13，未来传输适配仅需更新本插件。
